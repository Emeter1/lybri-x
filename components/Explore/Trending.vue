...
const tabs = ['All', 'Fiction', 'Sci-Fi', 'Mystery'];
...
<template>
    <section class="py-8">
        <div class="mb-8">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-2 font-serif">Trending Now</h2>
            <p class="text-gray-600 text-lg mb-6">The most popular stories this week across all genres</p>

            <!-- Tabs -->
            <div class="flex flex-wrap gap-2">
                <button v-for="tab in tabs" :key="tab" @click="activeTab = tab"
                    class="px-6 py-2 rounded-lg border transition-colors font-medium text-sm" :class="[
                        activeTab === tab
                            ? 'bg-[#FFF9E5] border-[#FFE580] text-gray-900'
                            : 'bg-white border-white text-gray-600 hover:bg-gray-50'
                    ]">
                    {{ tab }}
                </button>
            </div>
        ...
        <!-- Grid: cols-1 mobile, cols-2 tablet, cols-3 desktop (wrapping 4th item) -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="book in trendingBooks" :key="book.id" class="bg-white rounded-lg overflow-hidden group">
                <!-- Image Container -->
                ...
                <div class="space-y-3">
                    ...
                    <!-- Tags -->
                    <div class="flex items-center gap-2 text-xs font-medium text-gray-900">
                        <span v-for="(tag, index) in book.tags" :key="tag">
                            {{ tag }}<span v-if="index < book.tags.length - 1" class="mx-1 text-gray-400">•</span>
                        </span>
                    </div>
